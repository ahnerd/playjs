...
(function(w) {
	...
	var rSpace = /^[\s\u00A0]+|[\s\u00A0]+$/g,
		...
		p = namespace('Py', {
		    ...
		    setupWindow: function(w) {
				...
				String.map('$ ready Class addEventListener removeEventListener using data setData namespace dataIf', p, w, true);
	...
	function Native(nativeClass) {
		...
		// 简单拷贝  Class.prototype 的成员，即拥有类的特性。
		// 在 JavaScript， 一切函数都可作为类，故此函数存在。
		// Class.prototype 的成员一般对当前类构造函数原型辅助。
