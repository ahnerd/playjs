...
(function(w) {
	...
	var rSpace = /^[\s\u00A0]+|[\s\u00A0]+$/g,
		...
		p = namespace('Py', {
			 ...
			 * @param {String} uri 地址。
			 * @return {Object} 对象。
			 * @see Py.loadText, Py.using
			 */
			loadScript: function(uri) {
				
				// 加载文本，回调函数是执行。
				return p.loadText(uri, p.eval);
			},
	...
	function using(name) {
		...
		assert(name, "name不是合法的字符串");
		
		// 处理相应的文件。
		var fn = name.toLowerCase().replace(rPoint, '/') + ".js";
		
		// 不存在就载入。
		return p.namespaces.include(name) || !o.each(document.getElementsByTagName("script"), function(node) {
			return node.src.indexOf(fn) == -1;
		}) || p.loadScript(p.rootPath + fn);
	}
	...
	function imports(resource, theme){
		if(Py.namespaces.include(resource))
			return;
			
		 if(resource.indexOf('/') == -1){
		 	resource = p.rootPath + resource.toLowerCase().replace(rPoint, '/') + '.css';
		 }
		 
		 if(resource.indexOf('*') > -1){
		 	return (theme || ['share', Py.theme]).forEach(function(value){
				imports(resource.replace('*', value));
			});
		 }
		 
		 if(Object.each(document.styleSheets, function(css){
		 	return css.href !== resource;
		 })){
		 	var link = document.createElement('link');
			link.href = resource;
			link.rel = 'stylesheet';
			link.type = 'text/css';
		 	(document.getElementsByTagName('head')[0] || document).appendChild(link);
		 }
	}
